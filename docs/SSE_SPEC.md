...
| **CAUTIOUS_SPECULATIVE, 검증 실패 → 교체/위기**              | `session_meta` → `delta` × N (일부 또는 전체) → `delta.replace`(대체 문구) → `done`(`finished_reason="replaced_by_guard"`, `is_crisis_flagged`는 true/false 모두 가능) | ⚠️ 이 경로에서 `is_crisis_flagged=true`가 나와도 **`crisis` 이벤트는 전송되지 않는다** (7장 참고)                                    |
...
- **`is_crisis_flagged=true`인데 `crisis` 이벤트가 없을 수 있다.** `CRISIS_FLOW` 결정이 메시지 처리 *시작 시점*에 내려지거나 `BUFFER` 모드에서 내려지면 `crisis` 이벤트(+hotline 정보)가 함께 오지만, `CAUTIOUS_SPECULATIVE` 모드 스트리밍 중 비동기 검증에서 위기로 재분류되는 경우(`OutputJudge`의 `CRISIS_FLOW` 액션)는 `delta.replace` + `done(is_crisis_flagged=true, finished_reason="replaced_by_guard")`만 오고 `crisis` 이벤트는 오지 않는다 (현재 백엔드 구현상의 비대칭이며, 향후 바뀔 수 있음). **위기 UI(상담 전화 안내 등)를 노출할지 여부는 `crisis` 이벤트 수신이 아니라 `done.is_crisis_flagged`를 기준으로 판단**하고, `crisis` 이벤트가 없을 경우를 대비한 기본 안내 문구/번호를 프론트엔드에 하드코딩해두는 것을 권장한다.
